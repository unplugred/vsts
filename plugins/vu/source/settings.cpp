...
VUAudioProcessorSettings::VUAudioProcessorSettings(VUAudioProcessor& p, int paramcount, knob* k) : audio_processor(p), knobs(k), DocumentWindow("VU Settings", Colours::black, DocumentWindow::closeButton), plugmachine_gui(*this, p, 200, (16+PADDING*2+MARGIN*2)*(paramcount+2)+30, 1, 1, false, false) {
...
VUAudioProcessorSettings::~VUAudioProcessorSettings() {
	audio_processor.settingswindow = nullptr;
	close();
}
void VUAudioProcessorSettings::closeButtonPressed() {
	delete this;
}

void VUAudioProcessorSettings::newOpenGLContextCreated() {
	add_font(&font);
...
void VUAudioProcessorSettings::renderOpenGL() {
	draw_begin();
	...
	for(int i = 0; i < knobcount; i++) {
		...
		knobshader->setUniform("fill",knobs[i].normalize(knobs[i].value));
		glDrawArrays(GL_TRIANGLE_STRIP,0,4);
		++knoby;
	}
	knobshader->setUniform("knobpos",((float)MARGIN)/getWidth(),1-((knoby+1.f)*(font.line_height+PADDING*2+MARGIN*2)-MARGIN)/getHeight());
	...
	for(int i = 0; i < knobcount; i++) {
		font.shader->setUniform("fill",knobs[i].normalize(knobs[i].value));
...
void VUAudioProcessorSettings::openGLContextClosing() {
	draw_close();
}
void VUAudioProcessorSettings::paint(Graphics& g) { }

void VUAudioProcessorSettings::timerCallback() {
...
void VUAudioProcessorSettings::parameterChanged(const String& parameterID, float newValue) {
	for(int i = 0; i < knobcount; i++) if(knobs[i].id == parameterID) {
		return;
	}
}
void VUAudioProcessorSettings::mouseMove(const MouseEvent& event) {
	int prevhover = hover;
	hover = recalc_hover(event.x,event.y);
	if(hover == -2 && prevhover != -2 && websiteht <= -1) websiteht = .65f;
}
void VUAudioProcessorSettings::mouseExit(const MouseEvent& event) {
	hover = -1;
}
void VUAudioProcessorSettings::mouseDown(const MouseEvent& event) {
	if(dpi < 0) return;
	initialdrag = hover;
	if(hover > -1) {
		initialvalue = knobs[initialdrag].normalize(knobs[initialdrag].value);
		valueoffset = 0;
...
void VUAudioProcessorSettings::mouseDrag(const MouseEvent& event) {
	if(initialdrag > -1) {
		if(knobs[initialdrag].isbool) {
			hover = recalc_hover(event.x,event.y);
			hover = initialdrag==hover?initialdrag:-1;
			if(knobs[initialdrag].normalize(knobs[initialdrag].value) != (initialdrag==hover?(1-initialvalue):initialvalue))
...
void VUAudioProcessorSettings::mouseUp(const MouseEvent& event) {
	if(dpi < 0) return;
	if(initialdrag > -1) {
		...
		if(hover > -1) {
			audio_processor.undo_manager.setCurrentTransactionName(
				(String)((knobs[initialdrag].normalize(knobs[initialdrag].value)-initialvalue)>=0?"Increased ":"Decreased ") += knobs[initialdrag].name);
			audio_processor.undo_manager.beginNewTransaction();
		}
		if(!knobs[initialdrag].isbool) {
		...
		for(int i = 0; i < knobcount; i++) {
			...
			audio_processor.props.getUserSettings()->setValue(knobs[i].id,knobs[i].value);
			audio_processor.pots[i].defaultvalue = knobs[i].value;
		}
	} else {
		hover = recalc_hover(event.x,event.y);
		if(hover == -2) {
...
void VUAudioProcessorSettings::mouseDoubleClick(const MouseEvent& event) {
	if(hover <= -1) return;
...
void VUAudioProcessorSettings::mouseWheelMove(const MouseEvent& event, const MouseWheelDetails& wheel) {
	if(hover <= -1 || knobs[hover].isbool) return;
	audio_processor.apvts.getParameter(knobs[hover].id)->setValueNotifyingHost(
		knobs[hover].normalize(knobs[hover].value)+(wheel.deltaY>0?1.f:-1.f)*fmax(1.f/(knobs[hover].maximumvalue-knobs[hover].minimumvalue),fabs(wheel.deltaY)*((event.mods.isShiftDown() || event.mods.isAltDown())?.03f:.2f)));
}
int VUAudioProcessorSettings::recalc_hover(float x, float y) {
	int knoby = 0;
	for(int i = 0; i < knobcount; i++) {
...
void VUAudioProcessorSettings::moved() {
