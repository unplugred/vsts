...
DietAudioAudioProcessorEditor::DietAudioAudioProcessorEditor(DietAudioAudioProcessor& p, int paramcount, pluginpreset state, pluginparams params) : audio_processor(p), plugmachine_gui(p, 30+106*2, 70+100*2) {
	for(int x = 0; x < 2; ++x) {
		for(int y = 0; y < 2; ++y) {
			int i = x+y*2;
...
void DietAudioAudioProcessorEditor::calcvis() {
	...
	for(int i = 0; i < 226; ++i) {
		...
		visline[i*2+1] = 1-(62+0*38)/(height*.5f);
	}
}
