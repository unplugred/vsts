...
SucroseAudioProcessor::SucroseAudioProcessor() : apvts(*this, &undo_manager, "Parameters", create_parameters()),
												 ...
												 dsp(DspEngine(0, 44100))
{
	...
	for (int i = 0; i < paramcount; i++)
	{
		...
		if (params.pots[i].smoothtime > 0)
			params.pots[i].smooth.setCurrentAndTargetValue(state.values[i]);
...
double SucroseAudioProcessor::getTailLengthSeconds() const { return 0.01; }
...
{
	if (channelnum <= 0 || samplesperblock <= 0)
		return;
	...
	if (dsp.channels() != channelnum || dsp.sample_rate != samplerate)
	{
		dsp = DspEngine(channelnum, samplerate);
	}
	else
	{
		dsp.reset(); // skip alloc if we can
	}
}
...
{
	for (int i = 0; i < paramcount; i++)
		if (parameterID == params.pots[i].id)
		{
			if (params.pots[i].smoothtime > 0)
