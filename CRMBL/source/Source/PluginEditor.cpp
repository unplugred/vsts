...
void main(){
	...
	textshader->addVertexShader(audioProcessor.logger.textvert);
...
void main(){
	...
	blackshader.reset(new OpenGLShaderProgram(context));
	blackshader->addVertexShader(blackvert);
	blackshader->addFragmentShader(blackfrag);
	blackshader->link();
...
void CRMBLAudioProcessorEditor::renderOpenGL() {
	...
	if(creditsalpha >= 1) {
		...
		audioProcessor.logger.drawstring((String)"Temporary user interface! The look of this\nVST is subject to change. Compatibility\nwith upcoming versions is not guarenteed.",0,0,0,1,&textshader);
...
void CRMBLAudioProcessorEditor::calcvis() {
	for(int i = 0; i < 226; i++) {
		visline[i*2] = (i/112.5f)*(1-(16.f/getWidth()))+(16.f/getWidth())-1;
		visline[i*2+1] = 1-(62+audioProcessor.pnch(sin(i/35.8098621957f)*.8f,knobs[5].value)*38)/(getHeight()*.5f);
	}
}
